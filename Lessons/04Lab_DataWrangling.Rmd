...
NTL.phys.datafil <- filter(NTL.phys.data, year4 ==1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data3 <- filter(NTL.phys.data, lakename == "Tuesday Lake" & year4 %in% c(1990:1999))
NTL.phys.data3
```
...
NTL.phys.data.july <- filter(NTL.phys.data, lakename == "Tuesday Lake", year4 %in% c(1990:1999), month(sampledate) == 07)
...
NTL.phys.data.july.mean <- NTL.phys.data.july |> filter(depth == 0) |> summarise(mean = mean(temperature_C))
NTL.phys.data.july.mean
...
meantemp_pp <- NTL.phys.data |> filter(lakename == "Peter Lake" | lakename == "Paul Lake")|> 
  filter(depth == 0) |>
  filter(!is.na(temperature_C)) |>
  group_by(lakename) |>
  summarise(meantemps = mean(temperature_C), .groups = "drop")
meantemp_pp
