...
years90to99 <- filter(NTL.phys.data, year4 %in% c(1990:1999))
...
tempdesc <- arrange(NTL.phys.data, desc(temperature_C))
...
```{r mutating data}
farenheit <- mutate(NTL.phys.data.temps, temperature_F = (temperature_C*9/5) + 32)
...
write.csv(
 processedpeter, 
  row.names = FALSE,
  file = here("Data/Processed/processedpeter.csv")
)
```
